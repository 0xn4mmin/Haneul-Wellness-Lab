-- Add the previous measurement value so the app can show "this week" progress
-- (latest step) alongside overall progress.
  ...
  select coalesce(jsonb_agg(r), '[]'::jsonb) into result from (
    select jsonb_build_object(
      ...
      'prev', (select value from public.metric_readings mr2
                where mr2.user_id = g.user_id and mr2.metric_key = g.metric_key
                order by mr2.date desc offset 1 limit 1)
    ) as r
