...
## 🤖/👤 6. (나중) 인바디 결과지 OCR — Railway

M4 단계. 지금은 보류. 진행 시:
- 🤖 OCR 워커 서비스(Node) 스캐폴딩 + Supabase 연결 코드 작성.
- 👤 Railway 프로젝트 생성, OCR 제공자(예: Google Vision/CLOVA OCR) API 키 발급,
  Railway 환경변수 등록.
- 🤖 결과지 업로드 → 파싱 → `measurements`/`metric_readings` 초안 생성 → 검수 플로우.
